...
export default function Cliente() {
  const { nome, telefone, email, placaVeiculo, modeloVeiculo } =
  ...
  const handleDeleteCliente = (clienteKey) => {
    ...
    const listaClientes = clientes.filter(
      (cliente) => cliente.key !== clienteKey
    );
    localStorage.setItem("clientes", JSON.stringify(listaClientes));
    setClientes(listaClientes);
  };
  ...
  return (
    <div>
      ...
      <div className="identificacao-cliente">
      ...
      <div className="qr-code-and-list">
        ...
        <div className="clientes-cadastrados">
          ...
          <ul>
            {clientes.map((cliente) => (
              <li key={cliente.key}>
                <button onClick={() => handleDeleteCliente(cliente.key)}>
                  Excluir
                </button>
